# NomiCheck — Software Design Document · v2.5
...
| Metadato | Valor |
|---|---|
| Versión | 2.5 — pago on-chain no-custodial de contratistas en USDC/Base (BatchPago + PagoItem, tasa TRM con hash de snapshot, verificación por txHash con viem); salarios de empleados quedan siempre en COP (CST art. 134-136). Base parametrizable para expansión país (mismo mecanismo `ReglaLegal`). v2.4 — prestaciones sociales completas (cesantías, intereses, prima, vacaciones) en scope; `CalculadoraPrestaciones` + provisiones mensuales en modo empresa; documentación de escenarios edge con tests Vitest |
...
> **Nota de transición (v2.5 — Fase 8, pago on-chain):** se agrega una capa de **pago no-custodial** en USDC nativo Circle sobre Base (chainId 8453, contrato `0x833589FCD6eDb6E08f4c7C32D4f71b54bdA02913`) para cerrar el ciclo `liquidado → pagado`. NomiCheck **nunca custodia fondos ni firma transacciones**: `pagosService.generarBatchPago()` congela la tasa COP→USDC (TRM oficial de datos.gov.co + prima parametrizable, con hash sha256 del snapshot para trazabilidad) y produce dos artefactos de firma que el empleador ejecuta desde su propia wallet — links **EIP-681** uno-a-uno y un **Safe Transaction Builder JSON** de batch. `verificarBatchPago(txHash)` lee el receipt vía **viem** contra un RPC de Base, decodifica los `Transfer` del contrato USDC y confirma match exacto de destino+valor por item; batch verificado → periodo `pagado` (transición con `actualizarPeriodoConVersion`, concurrencia optimista). Legalmente **fase 1 solo contratistas** (Ley 1819/2016 art. 244, pago comercial); los `Empleado` reciben salario en COP (CST art. 134-136 exige moneda legal) y su `walletAddress` es opcional, solo como constancia — nunca genera pago. Alcance país queda parametrizable (`ReglaLegal` con claves `pais_moneda`, `pago_onchain_*`) para expansión.
>
> **Nota de transición (v2.4 — continuación de v2.3):** se refactorizó el core de liquidación para soportar escala enterprise (1000+ empleados). La liquidación pasó de ser una transacción síncrona HTTP a un proceso asíncrono en background mediante **pg-boss** (colas sobre el mismo Postgres de Supabase — sin Redis), procesando en lotes de 50 empleados con concurrencia optimista en las transiciones de estado. Se abandona la compuerta de QA "todo o nada" en favor de un estado de liquidación parcial (`liquidado_con_rechazos`) para no frenar la nómina de mil personas por un solo recibo con errores.
>
...
| Motor de reglas legales versionadas por fecha (SMLMV, recargos, jornada, aportes) | Generar el archivo/formato de planilla que se sube a un operador PILA (Aportes en Línea, SOI, etc.) — los valores exactos por IBC sí están en scope, ver §13 |
...
| **Pago on-chain no-custodial de contratistas** en USDC nativo sobre Base (EIP-681 + Safe batch generados por el servidor; empleador firma desde su wallet; verificación por txHash con viem) | Custodia de fondos / hot wallets / firma server-side (bloqueado por diseño — nunca en scope) |
| **Multi-país parametrizable** vía `ReglaLegal` (misma mecánica de vigencias) — catálogo actual es Colombia | Salario de `Empleado` en cripto (CST art. 134-136: moneda legal COP) — `walletAddress` en `Empleado` es solo constancia opcional |
...
| **pg-boss** | Background jobs / colas | Cola sobre el mismo Postgres de Supabase (schema propio `pgboss`, `SKIP LOCKED` + `LISTEN/NOTIFY`). Worker de liquidación asíncrona registrado en el mismo proceso del API (`workers/liquidacionWorker.ts`); escala horizontal = N réplicas del contenedor. |
...
| **viem** | Cliente EVM (verificación on-chain) | Lee receipts y decodifica logs `Transfer` del contrato USDC vs los `PagoItem` del batch. RPC de Base parametrizable por env (`BASE_RPC_URL`, default `https://mainnet.base.org`). NO se usa para firmar — el servidor no tiene llaves privadas |
| Vitest | Tests | Motor de reglas con los 2 comprobantes reales como regresión + suite de pagos (`pagosService`) sin BD ni RPC |
...
| **Custodia de fondos / hot wallets / firma server-side** | El servidor **jamás** guarda llaves privadas ni mueve dinero. El pago on-chain es **no-custodial** (§17): el servidor genera artefactos (EIP-681 + Safe JSON) y verifica receipts; el empleador firma desde su propia wallet. Evita licencia SFC y riesgo de custodia. |
| **Bridged tokens / multi-red** | Fase 1 solo acepta el par `{red: "base", token: "USDC"}` con validación estricta (`resolverRedPago` lanza `ErrorRedNoSoportada` en cualquier otro par). USDC nativo Circle, no USDbC bridged. Multi-red vive en `ReglaLegal` como configuración futura, no como refactor. |
| **Redis / BullMQ** | El volumen esperado del sistema de colas (liquidaciones de periodo, no miles de eventos por segundo) se absorbe perfectamente con `pg-boss` sobre la misma conexión de Postgres que ya usa Prisma. Añadir Redis obliga a introducir un servicio nuevo en el `docker-compose`, otra pieza gestionada en producción y otro punto de fallo — sin ganancia real a este volumen. Mantiene la simplicidad "una imagen, un contenedor" del §05. |
...
| `walletPagadora` | string? | Dirección EVM 0x… desde la que la empresa firma los batches on-chain. Referencial (permite verificar el `from` esperado); NomiCheck no la controla ni la exige — un batch puede firmarse desde cualquier wallet |
...
| `walletAddress` | string? | **Solo constancia complementaria — NUNCA genera pago on-chain.** El salario legal se paga en COP por el CST art. 134-136. La wallet queda en el perfil como referencia (portal colaborador, eventual constancia de recibo firmado); `Empleado` está explícitamente excluido de `generarBatchPago` |
...
| `estado` | string | enum: `borrador` · `liquidando` · `liquidado_con_rechazos` · `liquidado` · `fallido` · `pagado`. Solo `borrador` acepta turnos/ediciones. `liquidando` es transitorio (job en vuelo, bloquea edición y la reversión); `liquidado_con_rechazos` = liquidación parcial (algunos empleados fallaron QA, quedan en `erroresLiquidacion` sin recibo); `fallido` = excepción no-QA en el worker. Los tres terminales (`liquidado`, `liquidado_con_rechazos`, `fallido`) son reversibles a `borrador` |
| `version` | int | Concurrencia optimista: las transiciones de estado exigen `where {id, version}` + `version:{increment:1}`; los updates de `progreso` intermedios NO usan `version` (solo el job dueño del `jobId` escribe mientras `estado='liquidando'`). Prisma `P2025` → `ErrorConflicto` → HTTP 409 |
| `jobId` | string? | ID del job en pg-boss. Null hasta que se encola; se rellena al pasar a `liquidando` y se limpia al `revertirABorrador` |
| `progreso` | int | 0–100. Se actualiza al cierre de cada lote del worker. El frontend hace polling cada 3s a `/estado` para leerlo |
| `erroresLiquidacion` | json? | Dos formas: `RechazoQA[]` (`{ empleadoId, nombre, issues[] }`) cuando `estado='liquidado_con_rechazos'`, o `{ mensaje, contexto }` cuando `estado='fallido'`. Se limpia al revertir |
| `notaEdicion` / `editadoEn` | string? / datetime? | Rastro de auditoría de la última edición de fechas (solo permitida en `borrador`) |
...
### `Contratista` — prestador de servicios (no laboral)
Existente desde v2.2 (§13). Gana en v2.5:
| Columna nueva | Tipo | Notas |
|---|---|---|
| `walletAddress` | string? | Dirección EVM 0x… donde recibe honorarios en USDC. **Opt-in**: sin wallet, el contratista queda `excluidosSinWallet` en el batch (no bloquea la generación del lote de los demás). Validación de formato `/^0x[a-fA-F0-9]{40}$/` en zod y UI |

### `BatchPago` — lote de pago on-chain de un periodo (v2.5, Fase 8)
| Columna | Tipo | Notas |
|---|---|---|
| `id` | PK | — |
| `empresaId` / `periodoId` | FK | Un periodo puede tener varios batches (uno vigente + intentos expirados/fallidos como historial) |
| `estado` | string | enum: `generado` · `expirado` · `verificado` · `fallido_verificacion`. Se marca `expirado` de forma perezosa al leer si `expiraEn < now()`; `verificado` viene de `verificarBatchPago` cuando todos los items matchean on-chain |
| `red` / `token` / `tokenAddress` | string | Fase 1: siempre `("base", "USDC", "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913")`. Validación estricta contra `resolverRedPago` en `pagosConfig.ts` — cualquier otro par lanza `ErrorRedNoSoportada` |
| `tasaSnapshot` | Json | `{ trm, fuente, fechaTrm, primaPct, tasaEfectiva, capturadoEn, hash }` — el `hash` sha256 sobre el JSON canónico permite a terceros verificar la tasa citada sin confiar en el servidor |
| `totalCop` / `totalUsdc` | float | `totalUsdc` con 6 decimales (unidades del contrato USDC) |
| `txHash` / `verificadoEn` | string? / datetime? | Se llenan al verificar exitosamente |
| `generadoEn` / `expiraEn` | datetime | Ventana `pago_onchain_ventana_horas` (default 6h). Vencida → regenerar con tasa fresca |
| Trigger de auditoría | — | `auditoria_BatchPago` usa `fn_auditar_cambio('empresaId')` (mismo patrón que las demás tablas multi-tenant) |

### `PagoItem` — línea del batch por contratista
| Columna | Tipo | Notas |
|---|---|---|
| `id` | PK | — |
| `batchId` | FK → `BatchPago` | Cascada al eliminar el batch |
| `reciboId` | FK → `ReciboPago` | El recibo del contratista que este item paga |
| `destinoWallet` | string | Copia del `Contratista.walletAddress` **al momento de generar** — un cambio posterior no afecta lotes ya generados |
| `montoCop` / `montoUsdc` | float | `montoUsdc = redondear(neto / tasaEfectiva, 6 decimales)`, half-away (Math.round de unidades) |

> **Verificación on-chain** (`pagosService.verificarBatchPago`): con **viem** contra RPC de Base (`BASE_RPC_URL`), lee el receipt del `txHash`, decodifica los logs `Transfer` del `tokenAddress` con `parseEventLogs`, y para cada `PagoItem` busca **un log** con `to === destinoWallet` y `value === copAUnidadesUsdc(montoCop, tasaSnapshot.tasaEfectiva)`. Los logs se marcan como "consumidos" para no reutilizarlos entre items (dos items al mismo destino requieren dos transfers reales). Todos confirmados → `verificado` + transición del periodo a `pagado` vía `actualizarPeriodoConVersion` (concurrencia optimista). Faltantes → `fallido_verificacion` con detalle por item.
...
| `POST /api/empresa/periodos/:id/liquidar` | admin_empresa | **Encola** la liquidación asíncrona y responde **`202 Accepted`** con `{ jobId, estado: "liquidando" }`. El trabajo pesado corre en background (pg-boss + `workers/liquidacionWorker.ts`, lotes de 50). El frontend consulta el estado por polling — ver `/estado` |
| `GET /api/empresa/periodos/:id/estado` | los 3 roles | Endpoint de solo lectura para el polling corto del frontend — retorna `{ estado, progreso, jobId, erroresLiquidacion, version }` |
...
| `POST /api/empresa/periodos/:id/batch-pago` | admin_empresa | **Genera lote de pago USDC** (v2.5). Periodo debe estar en `liquidado` o `liquidado_con_rechazos`. Congela tasa TRM+prima con hash, calcula items para contratistas con wallet, persiste `BatchPago` + `PagoItem[]` vía `conAuditoria`. Responde 201 con lote + `items[]` con `linkEip681`, `safeBatch` (JSON del Safe Transaction Builder), `excluidosSinWallet[]`, `disclaimer` |
| `GET /api/empresa/periodos/:id/batch-pago` | los 3 roles | Lote vigente + estado; marca `expirado` perezosamente si venció |
| `POST /api/empresa/batches/:batchId/verificar` | admin_empresa | `{ txHash }` → verificación on-chain contra RPC de Base (viem). 200 con `estado: "verificado"` → periodo pasa a `pagado`. 422 con detalle por item si falta algún transfer |
...
2. Al dar "Liquidar", el periodo pasa a estado `liquidando`. El motor procesa a los empleados en background (lotes de 50 vía pg-boss, cada lote una transacción independiente envuelta en `conAuditoria`). El frontend muestra el progreso en tiempo real consultando `/estado` cada 3s y bloquea la edición del periodo mientras corre el proceso.
3. Si todos pasan QA, el estado pasa a `liquidado`. Si hay errores aislados, pasa a `liquidado_con_rechazos`, guardando el log detallado de quién falló y por qué en `erroresLiquidacion`, y generando los `ReciboPago` de los que sí fueron exitosos. Si el worker cae por una excepción no-QA (DB caída, dato inesperado), el estado queda `fallido` con `{ mensaje, contexto }` — los lotes ya cerrados permanecen persistidos y un revert-reencolar continúa sin duplicarlos (guard de idempotencia por `(periodoId, empleadoId)`).
4. Dashboard muestra totales del periodo; los colaboradores con cuenta ven su recibo nuevo en su portal.
5. **Pago on-chain de contratistas** (v2.5, opcional — solo si hay contratistas con wallet): en un periodo `liquidado`/`liquidado_con_rechazos`, el admin abre `PanelPagoOnChain` y da "Generar lote de pago USDC". El servidor congela la tasa TRM del día + prima con hash y devuelve links **EIP-681** (uno por contratista) + **Safe Transaction Builder JSON** (batch de todo en una sola tx). El empleador firma desde su propia wallet (NomiCheck nunca toca fondos), pega el `txHash`, y la verificación con viem confirma los transfers → el periodo pasa a `pagado`. Empleados quedan siempre en flujo de pago COP (fuera de este paso).
...
| Reglas legales = datos versionados | Nunca constantes en código; vigencias por fecha, sin solapes, con fuente. El **spec humano** vive en `sdd/vault/` (baúl markdown con reglas estables en 01–04 y 06, tabla maestra ÚNICA de valores actualizables en `05_Valores_Actualizables.md`, y el mapa clave↔regla↔código en `07_Trazabilidad_Codigo.md`); el catálogo `ReglaLegal` es la implementación de ese spec — deben coincidir en cada revisión, y `vaultSincronia.test.ts` falla si se separan. `REGLAS_VERIFICADAS_AL` (en `reglasVerificadasService.ts`, de donde lo importan los cuatro servicios de batch) cita la fecha de última revisión y viaja en cada output del wrapper stateless |
...
| Locking del progreso de liquidación | Las transiciones de estado del periodo (`borrador`→`liquidando`→`liquidado`/`liquidado_con_rechazos`/`fallido` y reversos) exigen concurrencia optimista (`version` en el `where`; P2025→409). Los updates internos de `progreso` durante el job son escrituras directas del proceso dueño del `jobId` — sin chequeo de versión, porque solo ese proceso escribe mientras `estado='liquidando'`. Mezclar `version` en cada tick de progreso generaría 409 falsos con cualquier polling concurrente |
| **Pago on-chain es no-custodial, siempre** | NomiCheck jamás guarda llaves privadas ni firma transacciones. Genera artefactos (EIP-681, Safe JSON), verifica receipts. Custodia queda fuera de scope permanentemente (evita licencia SFC y elimina el riesgo de fondos ajenos en el servidor) |
| **USDC nativo Circle, no bridged** | Fase 1 acepta *solamente* `{red: "base", token: "USDC"}` con `resolverRedPago` estricto. Multi-red (Polygon, Arbitrum, Solana) es futuro parametrizable en `ReglaLegal`, no refactor de código |
| **Salario de empleados: siempre COP** | CST art. 134-136 exige pago en moneda legal. `Empleado.walletAddress` es opcional y solo referencial (constancia); `pagosService.generarBatchPago` está codificado para excluir `Empleado` y solo iterar `Contratista` (Ley 1819/2016 art. 244, pago comercial) |
| **Tasa COP→USDC con hash de trazabilidad** | El batch congela `tasaSnapshot` con TRM oficial + prima parametrizable y sha256 sobre JSON canónico. Un tercero puede reconstruir el hash con los mismos insumos y verificar que la tasa citada es la real. La ventana `pago_onchain_ventana_horas` fuerza regeneración si no se firma a tiempo — protege al empleado de un lote firmado con tasa vieja |
| Compuerta de QA a escala: liquidación parcial | Un error de QA en un empleado (`IBC_FUERA_DE_RANGO`, `NETO_BAJO_MINIMO`, etc.) excluye a ese empleado pero permite liquidar y persistir al resto del lote. El estado final `liquidado_con_rechazos` refleja este resultado parcial y guarda el detalle de los rechazados en `erroresLiquidacion`. No se vuelve al comportamiento "todo o nada" del pre-async — a 1000+ empleados, un dato malo en una fila no puede bloquear la nómina del resto |
...
- [x] **Liquidación PILA exacta por periodo + semáforo de cumplimiento** (cierra dos pendientes de §14). Decisiones de alcance confirmadas con el usuario antes de implementar:
  - **PILA "exacta" = valores exactos por IBC real, sin generar el archivo de planilla**: generar el formato que se sube a un operador PILA autorizado (Aportes en Línea, SOI, etc.) requeriría un catálogo completo de códigos de EPS/AFP/ARL/cajas de todo el país — fuera de alcance a propósito, advertido explícitamente en la UI.
  - **Nuevo `Empleado.claseRiesgoArl`** (`Int @default(1)`, migración `20260720130000_empleado_clase_riesgo_arl`, solo dev): antes la clase de riesgo ARL era un parámetro de cálculo con default silencioso a clase 1 para todos (`costosService.ts` nunca la pasaba) — ahora se captura en el formulario de colaborador y se usa tanto en el estimado gerencial de Costos como en la PILA exacta.
  - **Motor** (`packages/reglas/src/costoEmpleador.ts`, nueva función `calcularLiquidacionPilaEmpleado()`): recibe el IBC y auxilio de transporte REALES de un `ReciboPago` ya liquidado (ya reflejan días trabajados, sin volver a prorratear) — los aportes se calculan directo sobre ese IBC, y las provisiones reutilizan la misma fracción 30/360 del estimado gerencial: como el IBC ya es proporcional a los días del periodo, el resultado sale correctamente prorateado sin que la función necesite saber cuántos días duró el periodo (propiedad verificada con tests: una quincena da exactamente la mitad de un mes completo). El umbral de exoneración (Ley 1607, 10 SMLMV) se evalúa contra el **salario mensual pactado**, no contra el IBC del periodo (que en una quincena sería la mitad y falsearía el umbral).
  - **Derivación del IBC real sin tocar el motor de liquidación**: `deduccionesDeLey()` (`deducciones.ts`) ya persiste el IBC exacto en el campo `base` de la línea "Salud (aporte empleado)" de cada `ReciboPago.lineas` — `pilaService.ts` lo lee de ahí en vez de sumar líneas de devengo (que podrían incluir conceptos extralegales que NO hacen IBC). Sin esa línea (aprendiz SENA etapa lectiva — SENA gestiona su afiliación) no hay IBC: ese recibo queda fuera de la PILA.
  - **Nueva advertencia del motor `advertenciaSalarioBajoMinimo()`** (`advertenciasContrato.ts`, conectada en ambas calculadoras): salario básico bajo el SMLMV en contrato de tiempo COMPLETO (indefinido/fijo/obra_labor) — nunca es legal, a diferencia de tiempo parcial (proporcional por diseño, tiene su propia advertencia de IBC/BEPS) o aprendizaje SENA (su pago es legalmente menor). Aparece también en los recibos reales de los colaboradores, no solo en el semáforo.
  - **Semáforo de cumplimiento NO duplica detección**: `cumplimientoService.ts` reusa las mismas funciones puras del motor (`advertenciaPatronAprendiz`, `advertenciaSalarioBajoMinimo`) contra los empleados activos actuales, y para horas extra excedidas (que depende de turnos de un periodo, no del `Empleado` en sí) escanea las advertencias YA persistidas en los `ReciboPago` de los últimos 6 periodos liquidados, buscando el substring fijo `"supera el máximo legal de"` que `calculadoraTurnos.ts` siempre usa — seguro porque la fuente es el propio motor, no texto libre de terceros. Nivel del semáforo: rojo si hay aprendices mal clasificados o salarios bajo el mínimo (riesgo legal/multas), amarillo si solo hay horas extra excedidas (menor, ya se paga completo), verde si no hay nada.
  - `packages/reglas/src/advertenciasContrato.ts` ahora se exporta desde el barrel del paquete (`index.ts`) — antes solo lo usaban las calculadoras internamente, `apps/api` no podía importarlo.
  - UI: `PilaEmpresa.tsx` (selector de periodo liquidado + desglose expandible por colaborador, mismo patrón visual que `CostosEmpresa.tsx`) y `CumplimientoEmpresa.tsx` (badge de semáforo + 3 secciones de alertas), nuevas pestañas "PILA" y "Cumplimiento" en `EmpresaApp.tsx`. Select de clase de riesgo ARL (I-V) agregado al formulario de colaborador en `DashboardEmpresa.tsx`.
  - Verificado: 16 tests de regresión nuevos en `@pv/reglas` (150/150 en el paquete — 6 de `calcularLiquidacionPilaEmpleado` con la propiedad de proporcionalidad quincena/mes, 4 de `advertenciaSalarioBajoMinimo`), `tsc` limpio en api/web, script de servicios end-to-end contra el contenedor (crear 3 empleados con distintas condiciones → semáforo detecta aprendiz mal clasificado y salario bajo mínimo correctamente antes de liquidar nada → crear y liquidar un periodo → PILA exacta coincide peso a peso con el cálculo manual esperado, incluida la clase de riesgo ARL real de cada empleado → PILA de un periodo en borrador rechazada con 422). Verificado también en navegador real con una empresa de prueba: semáforo muestra badge rojo con las 2 alertas exactas, PILA muestra el desglose completo con cada línea citando su ley. Datos de prueba limpiados; confirmado que las empresas reales quedaron intactas.
- [x] **B2B enterprise, pilar 2 — Motor de QA de nómina pre-pago** (SDD §15): módulo determinista `packages/reglas/src/qa/` — `IssueQA` tipado (código estable `HORAS_EXTRA_EXCEDIDAS_DIA`/`_SEMANA`, `TOPE_DEDUCCIONES_SUPERADO`, `NETO_BAJO_MINIMO`, `IBC_FUERA_DE_RANGO`, severidad, mensaje, `referenciaLegal`, `detalles.valorCalculado/valorLimite/contexto`), `ResultadoQA { estado: "aprobada"|"con_advertencias"|"rechazada", score, issues }` con fórmula de score documentada (100 − 25/error − 5/advertencia). `evaluarQA(datos, reglas)`: valida horas extra sobre `max_horas_extra_dia`/`_semana`, tope 50% del art. 149 (o solo advertencia si el tope se activó pero no se superó), rango IBC en `[1 SMLMV, ibc_tope_smlmv=25 SMLMV]` prorrateado por periodo (nueva clave sembrada en catálogo, fixtures y seed — Ley 100 art. 18 mod. Ley 797 art. 5, antes no existía en el motor), y neto ≥ SMLMV prorrateado tras deducciones ordinarias. Reusa `crearResolutorReglas`/`redondearPeso`/`diasEntreFechas` — no duplica lógica del motor. 6 tests nuevos en `qa.test.ts` (156/156 en el paquete).
- [x] **Gate de QA integrado a la liquidación** (SDD §15, pilar 2 — cierre): `liquidacionService.liquidarPeriodo()` corre `evaluarQA` sobre cada recibo ANTES de persistir. `rechazada` lanza `QaRechazadaError` con `{ empleadoId, nombre, issues[] }` — el controlador lo mapea a 422 con `{ error, rechazos }` (no confunde el body con un error 500 genérico); `con_advertencias` liquida pero persiste los issues en el nuevo campo `ReciboPago.qaIssues` (Json?, migración `20260720150000_recibo_qa_issues`, solo dev). Las señales que el QA necesita (excesos de horas extra por día/semana, activación del tope del art. 149) se extraen de las advertencias-string del motor con regex de formato estable en la capa de integración (no en el QA puro) — cuando las calculadoras migren a `IssueQA` nativo, esa capa desaparece sin tocar el QA. UI (`PeriodosEmpresa.tsx`): panel de rechazos por colaborador con código+mensaje+ley cuando la liquidación falla; badge amarillo con los issues persistidos en cada recibo aprobado con advertencias. Verificado end-to-end contra el contenedor: liquidación de un periodo limpio pasa con `qaIssues null`, un periodo con turnos de 18h/día lunes-sábado dispara `HORAS_EXTRA_EXCEDIDAS_DIA` (11 días) + `HORAS_EXTRA_EXCEDIDAS_SEMANA` (3 semanas) con las referencias legales correctas (D.L. 13/1967 y Ley 6/1981). Datos de prueba limpiados; empresas reales intactas.
- [x] **B2B enterprise, pilar 1 (parte A) — Roles granulares + Sede** (SDD §15, pilar 1): nuevos valores para `Usuario.rol`: `analista_rrhh` (opera sobre las sedes que le asignen; sin sedes se comporta como `admin_empresa`) y `auditor` (solo lectura de toda la empresa). Nuevo modelo `Sede` (empresaId + nombre, único por empresa) y `UsuarioSede` bridge (many-to-many, `onDelete: Cascade`) — migración `20260720170000_sedes_y_roles_granulares` (solo dev). `Empleado.sedeId` opcional (null = sin sede asignada, válido para empresas chicas). Middleware nuevo: `requiereEmpresaLectura` (los 3 roles) y `requiereEmpresaEdicion` (auditor cae con 403); helper `sedesDelUsuario()` que devuelve `number[] | null` — null = sin scoping. Rutas separadas por escritura/lectura; borrado, invitación y CRUD de sedes se restringen a `admin_empresa`. `empleadosService`: `listarEmpleados`/`actualizar`/`eliminar`/`retirar` aceptan opcional `sedes[]` — un analista con sedes solo ve/toca empleados de esas sedes (nuevo helper `empleadoAccesible` centraliza la verificación). Nuevo `sedesService` (crear/listar/eliminar sede — al borrar, los empleados quedan con `sedeId: null` y las asignaciones a analistas se cascadean) y flujo de "staff empresarial": `asignarStaff({ email, rol, sedeIds })` toma una cuenta ya registrada por su lado (Supabase Auth vive fuera; no se crean usuarios desde aquí — mensaje explícito si no existe) y la liga con rol + sedes; `quitarStaff` revierte a `empresaId: null, rol: "individual"`. UI: nueva pestaña "Sedes" (`SedesEmpresa.tsx`) con panel de sedes (crear/eliminar) y panel de staff (asignar por email con checkboxes de sedes cuando rol=analista); `FormEmpleado` gana selector de sede (solo aparece si la empresa tiene sedes creadas — no molesta a las empresas chicas). Verificado end-to-end contra el contenedor: crear 2 sedes → listarEmpleados scoped al analista solo devuelve empleados de su sede → analista bloquea write cross-sede con 422 → analista edita su empleado OK → quitarStaff limpia UsuarioSede → eliminar sede pone `sedeId: null` en sus empleados sin borrarlos. `tsc` limpio en api, build limpio en web. Datos de prueba limpiados; empresas reales intactas. **Pilar 1B (auditoría inmutable) — cerrado en el siguiente bullet.**
- [x] **B2B enterprise, pilar 1 (parte B) — Auditoría inmutable con trigger PL/pgSQL** (SDD §15, pilar 1B — cierre): nueva tabla `AuditoriaCambio` (`id BIGSERIAL`, `empresaId`, `usuarioId UUID`, `tabla`, `registroId TEXT`, `accion INSERT|UPDATE|DELETE` con CHECK, `valoresAnteriores JSONB`, `valoresNuevos JSONB`, `creadoEn`; migración `20260720190000_auditoria_inmutable`, solo dev). Función `fn_auditar_cambio()` genérica: lee `current_setting('app.usuario_actual', true)` como autor, serializa OLD/NEW con `to_jsonb`, y saca `empresaId` de la columna nombrada en `TG_ARGV[0]` (vacío para `ReciboPago`, que no tiene esa columna directa). Triggers `AFTER INSERT OR UPDATE OR DELETE` en `ReciboPago`, `PeriodoNomina` y `Empleado`. Inmutabilidad vía RLS solo-SELECT + `REVOKE UPDATE, DELETE` — capa que aplica al rol `authenticated` de Supabase; Prisma sigue siendo superuser y no debería escribir directo (documentado en la migración). Nuevo helper `conAuditoria(usuarioId, fn)` en `apps/api/src/lib/auditoria.ts` envuelve una transacción y setea `SELECT set_config('app.usuario_actual', usuarioId, true)` — SET LOCAL scoped a la transacción, sin residuo en el pool. Wiring: `liquidarPeriodo`, `revertirABorrador`, `crearEmpleado`, `actualizarEmpleado`, `eliminarEmpleado`, `retirarEmpleado` reciben `usuarioId` desde el controlador y ejecutan sus writes dentro de `conAuditoria`. Endpoint `GET /empresa/auditoria?limit` (accesible para los 3 roles) + `auditoriaService.listarAuditoria` que resuelve el nombre del usuario en un solo query y hace el join manual con `PeriodoNomina` para filtrar los `ReciboPago` (INSERT ó DELETE) por empresa. UI: nueva pestaña "Auditoría" (`AuditoriaEmpresa.tsx`) con listado colapsable — cada entrada muestra ícono por acción, tabla + registroId, timestamp local, autor con email, y al expandir un diff simple `Antes`/`Después` con el JSON completo. Verificado end-to-end contra el contenedor: crear/editar/eliminar un `Empleado` vía service con `conAuditoria` deja 3 entradas con `usuario = Ana Admin` en el orden correcto; una llamada directa a Prisma sin el wrapper deja `usuarioId null` (confirma que el trigger se dispara sí o sí, y que la ausencia de wrapper la detecta el auditor). `tsc` limpio en api, build limpio en web. Datos de prueba limpiados; empresas reales intactas.
- [x] **Advertencias del motor tipadas como `IssueQA` nativo — cierre de la migración parcial de §15 pilar 2**: `ResultadoNomina` gana `issues: IssueQA[]` en paralelo con `advertencias: string[]` (ambos coexisten; los consumidores actuales del string —semáforo, PILA, UI de badges— siguen funcionando sin cambios). Nuevos helpers `crearIssue(codigo, severidad, mensaje, ley, valorCalculado, valorLimite, contexto?)` y `emitirIssue(issues, advertencias, i)` en `packages/reglas/src/qa/index.ts` — las calculadoras las usan para empujar el string y el tipado a la vez. Migrado: `calculadoraTurnos` emite `HORAS_EXTRA_EXCEDIDAS_DIA`/`_SEMANA` con `detalles.contexto = fecha|semana`; `aplicarDeducciones` emite `TOPE_DEDUCCIONES_SUPERADO` (severidad `advertencia`) cuando recorta por CST art. 149; `calculadoraSalarioFijo` propaga los issues de `aplicarDeducciones` a través de `ensamblarResultado`. `DatosQA` reemplaza los campos ad-hoc `excesosHorasExtraDia/Semana/toperoDeduccionesActivado` por un único `issuesMotor: IssueQA[]` que `evaluarQA` inserta directo (sigue detectando `IBC_FUERA_DE_RANGO`/`NETO_BAJO_MINIMO`/superación explícita del 149 por sí mismo). En `liquidacionService.ts` desaparecen `RE_HE_DIA`/`RE_HE_SEMANA`/`MARCA_TOPE_DEDUCCIONES` y el helper `senalesQA()` — el gate ahora hace `evaluarQA({ ...datos, issuesMotor: resultado.issues })`. Verificado: 156/156 tests en `@pv/reglas` (dos aserciones nuevas embebidas en tests existentes de horas extra y recorte del art. 149 confirman el issue tipado con código+ley+contexto correctos); e2e contra el contenedor con un empleado que trabaja 18h/día L-S dispara `HORAS_EXTRA_EXCEDIDAS_DIA` + `HORAS_EXTRA_EXCEDIDAS_SEMANA` con `detalles.contexto = 2026-07-01…` sin pasar por regex. `tsc` limpio en api, build limpio en web. Datos de prueba limpiados; empresas reales intactas.
- [x] **B2B enterprise — liquidación asíncrona con pg-boss + gate QA por-empleado** (SDD §15, escalabilidad a 1000+ empleados): reemplazamos la liquidación síncrona (que arriesgaba timeout HTTP y bloqueaba el Event Loop a esa escala) por un pipeline background con estados terminales tipados. (a) **Máquina de estados** — `PeriodoNomina.estado` gana `liquidando`, `liquidado_con_rechazos` y `fallido` (sigue `String`, no enum nativo — mantiene el patrón). Nuevos campos `jobId String?`, `progreso Int @default(0)`, `erroresLiquidacion Json?`. Migración `20260722100000_periodo_liquidacion_asincrona` (solo dev). Vocabulario canónico en `apps/api/src/lib/estados.ts` (`ESTADOS_PERIODO`, `EstadoPeriodo`, `esEstadoPeriodo`, `ESTADOS_TERMINALES_LIQUIDACION`) — importado por controllers/services/tipos del frontend (espejo en `apiEmpresa.ts`). (b) **Cola sobre Postgres** — `pnpm add pg-boss ^12.26.2` en `apps/api`. Reusa la misma `DATABASE_URL` de Supabase (sin Redis; sigue "una imagen, un contenedor" del SDD §04). Singleton lazy `getBoss()`/`detenerBoss()` + cierre limpio en SIGTERM/SIGINT. Escalado horizontal = correr N réplicas del contenedor, pg-boss reparte vía `SKIP LOCKED`. (c) **Función pura extraída** — `apps/api/src/services/liquidacionCalculo.ts` con `calcularReciboLote(periodoId, periodo, empleados, turnos, reglas, festivos, resolutor)` y `calcularRecibosContratistas(...)` — reutiliza `CalculadoraPorTurnos`/`SalarioFijo`/`Servicios` + `evaluarQA` sin reimplementar. Aparte de facilitar el worker, esto vuelve testeable el cálculo por lotes sin BD. (d) **Worker por lotes** — `apps/api/src/workers/liquidacionWorker.ts` procesa `TAMANO_LOTE=50` empleados por transacción, cada una envuelta en `conAuditoria(usuarioId, tx)` (trigger sigue atribuyendo la escritura al humano correcto, no a un proceso anónimo). Idempotencia: filtra empleados que YA tienen `ReciboPago` para no violar `@@unique(periodoId, empleadoId)` en reintentos. Progreso 0-100 se actualiza al cerrar cada lote, sin `version` (solo el job dueño de `jobId` escribe mientras `estado='liquidando'`; las transiciones sí usan `version` para detectar reversiones concurrentes). (e) **Gate QA cambia de todo-o-nada a per-empleado** (decisión de diseño acordada con el usuario, ver `AskUserQuestion` en la sesión): un empleado `rechazada` NO tumba el periodo — queda reportado en `erroresLiquidacion` sin recibo, los demás sí liquidan. Estado terminal: `0 rechazos → liquidado`, `≥1 → liquidado_con_rechazos`, `excepción no-QA → fallido` con `{ mensaje, contexto }`. Se retiró `QaRechazadaError` + su 422; el frontend lee `erroresLiquidacion` del endpoint de estado. (f) **Endpoints** — `POST /empresa/periodos/:id/liquidar` responde `202 { jobId, estado: "liquidando" }` (antes: 200 con recibos). Nuevo `GET /empresa/periodos/:id/estado` (los 3 roles) devuelve `{ estado, progreso, jobId, erroresLiquidacion, version }` para polling. (g) **UI de progreso** — nuevo hook `usePeriodoEstado(periodoId, activo)` con polling cada 3s (SDD §04: push en vivo está fuera de scope, polling corto es la elección coherente) que se detiene en cualquier estado terminal. Nuevo componente `PanelLiquidacion.tsx` renderiza spinner + barra según estado (mint para `liquidando`, ámbar para `liquidado_con_rechazos` con lista de rechazos por empleado + issues+ley, rojo para `fallido`, verde para `liquidado`). `PeriodosEmpresa.tsx`: `liquidar()` ahora encola y confía en el polling; se retiró el `PanelRechazosQA` inline; el botón "Revertir" acepta los tres estados terminales (`liquidado`, `liquidado_con_rechazos`, `fallido`) y limpia `jobId`/`progreso`/`erroresLiquidacion` en la transición a `borrador`. Verificado end-to-end contra el contenedor: empresa temporal con 3 empleados (2 aprobados + 1 con `salarioBase=100` para forzar `IBC_FUERA_DE_RANGO`+`NETO_BAJO_MINIMO`) → encolado devuelve `jobId=edc884bb…` → polling: `t+1s liquidando progreso=0` → `t+2s liquidado_con_rechazos progreso=100`; recibos persistidos: 2 (Ana devengado $2.000.000 neto $1.840.000, Bob devengado $2.500.000 neto $2.300.000), Carlos sin recibo pero en `erroresLiquidacion` con ambos códigos+ley; reintento del mismo periodo (revertir + reencolar) NO duplica recibos (guard de idempotencia funciona). 160/160 tests en `@pv/reglas`, `tsc` limpio en api y web, API arranca con `[pg-boss] Worker registrado en cola "liquidar-nomina"`. Datos de prueba limpiados; empresas reales (id 6 Restaurante Resplandor dev y id 7 Ynt-abs) intactas. **Fuera de scope explícito** (misión 4 del prompt original): generador de archivo plano PILA — sigue como estaba en SDD §02, requiere catálogo completo de códigos EPS/AFP/ARL/caja que hoy no existe en el modelo; se abordará como Fase 2 separada.
- [x] **B2B enterprise — hardening: soft delete, concurrencia optimista y dos reglas nuevas de QA** (SDD §15, complemento de pilares 1 y 2): (a) **Soft delete de `Empleado`** — nuevo campo `Empleado.eliminadoEn DateTime?` (migración `20260721120000_empleado_soft_delete_periodo_version`, solo dev) que complementa (no reemplaza) el `activo=false` de retiro. `eliminarEmpleado` pasa de `prisma.empleado.delete` a `update({ eliminadoEn: now, activo: false })` — el guard existente (409 si hay recibos/turnos) se mantiene para dirigir esos casos a "Retirar". Filtros `eliminadoEn IS NULL` en `listarEmpleados`, `empleadoAccesible`, `liquidacionFinalService.liquidarFinal`, y los dos `count` de `periodosService` (edición de empleados y turnos por periodo). Índice parcial `Empleado_empresaId_eliminadoEn_idx WHERE eliminadoEn IS NULL` para las listas. (b) **Concurrencia optimista en `PeriodoNomina`** — nuevo campo `version Int @default(0)`, helper `actualizarPeriodoConVersion(tx, id, versionActual, data)` en `liquidacionService` que hace `update({ where: { id, version: X }, data: { ...d, version: { increment: 1 } } })` y captura `Prisma P2025` → `ErrorConflicto` (reutiliza el error tipado de `empleadosService`). Cablado en `liquidarPeriodo`, `revertirABorrador`, `editarPeriodo`. Controllers `editar`, `liquidar`, `revertir` mapean `ErrorConflicto` a HTTP 409 con mensaje "Otro usuario modificó este periodo…" (los clientes actuales tratan 4xx igual, así que no rompe UI existente). (c) **`INCOMPATIBILIDAD_NOVEDAD_TIEMPO`** — nuevo código en `qa/tipos.ts`. `DatosQA` gana `novedades?: { fecha, trabajo, remunerada? }[]`; `validarChoqueNovedades` agrupa por fecha y emite error si una misma fecha tiene a la vez `trabajo:true` y `trabajo:false` (guardarraíl defensivo — hoy los turnos de la BD solo son `trabajo:true`, pero cuando entre la entidad Incapacidad la regla ya protege). Cableado en `liquidacionService` pasando las novedades de turnos al `evaluarQA`. (d) **`DECIMALES_DETECTADOS_PILA`** — nuevo código, severidad `advertencia`. `validarDecimalesPila` revisa `totalDevengado`/`totalDeducciones`/`netoPagado`/`ibcPeriodo` y emite si `valor % 1 !== 0`. Red de seguridad ante bugs futuros: `redondearPeso` ya se aplica en cada línea del motor, pero un flotante colado no debe llegar silenciosamente a PILA (Resolución 2388 de 2016). Verificado: 10/10 tests en `qa.test.ts` (160/160 en el paquete), `tsc` limpio en api y web, API arranca sin errores, landing carga sin errores de consola. Sin cambios de UI (el badge de issues persistidos ya cubre los códigos nuevos automáticamente).
- [x] **Fase 8 — Pago on-chain no-custodial (USDC/Base)** (SDD §17, cierra el ciclo `liquidado → pagado` y activa el módulo BatchPagoOnChain del gap analysis de Execution Market). Alcance fase 1 confirmado con el usuario: **solo contratistas** (Ley 1819/2016 art. 244), USDC nativo Circle en Base con validación estricta, snapshot con hash. **(a) Schema** — `Empresa.walletPagadora`, `Contratista.walletAddress`, `Empleado.walletAddress` (solo constancia), nuevas tablas `BatchPago` + `PagoItem` con trigger `auditoria_BatchPago` (mismo patrón `fn_auditar_cambio('empresaId')`). Migración `20260723100000_pagos_onchain` (solo dev). **(b) Parametrización** — nuevas claves `ReglaLegal`: `pais_moneda`, `pago_onchain_red`, `pago_onchain_token`, `pago_onchain_token_address`, `pago_onchain_prima_pct` (default 0), `pago_onchain_ventana_horas` (default 6). Reusa el mecanismo existente de vigencias — la expansión a otro país entra como registros nuevos, no como refactor. **(c) `tasaCambioService.ts`** — obtiene TRM oficial de datos.gov.co (dataset 32sa-8pi3, sin API key), cache 1h, aplica prima parametrizable, hash sha256 sobre JSON canónico → trazabilidad verificable por terceros. **(d) `pagosService.ts`** — `generarBatchPago()` guard estado `liquidado`/`liquidado_con_rechazos`, filtra contratistas con wallet válida (los sin wallet quedan en `excluidosSinWallet`, no bloquean), calcula `unidades = Math.round(neto/tasa × 1_000_000)`, persiste `BatchPago` + `PagoItem[]` en `conAuditoria`, devuelve links **EIP-681** (`ethereum:<token>@<chainId>/transfer?address=<destino>&uint256=<unidades>`) + **Safe Transaction Builder JSON**. `verificarBatchPago()` con **viem** contra RPC de Base, `getTransactionReceipt` + `parseEventLogs` del erc20 Transfer, match exacto de destino+valor por item (logs consumidos, dos items al mismo destino requieren dos transfers reales); todos verificados → transición a `pagado` vía `actualizarPeriodoConVersion` (concurrencia optimista). **(e) Endpoints** — `POST /empresa/periodos/:id/batch-pago` (soloAdminEmpresa, 201), `GET /empresa/periodos/:id/batch-pago` (empresaLectura, marca `expirado` perezoso), `POST /empresa/batches/:batchId/verificar` (soloAdminEmpresa, 422 con detalle por item si fallido). **(f) UI** — captura de `walletAddress` en `FormContratista` con validación regex y estado visual (`ContratistasEmpresa.tsx`); nuevo `PanelPagoOnChain.tsx` (~230 líneas): botón generar, tabla items COP→USDC, snapshot con TRM+fecha+hash, links EIP-681 individuales + botón "Safe batch (todo en 1 tx)" con descarga JSON, cuenta regresiva de expiración, input txHash + "Verificar pago", detalle por destinatario si `fallido_verificacion`, disclaimer visible siempre. Renderizado desde `PeriodosEmpresa.tsx` cuando estado ∈ {liquidado, liquidado_con_rechazos, pagado}. **(g) Tests** — 10 tests unitarios en `pagosService.test.ts` (conversión COP→unidades USDC con redondeo half-away, hash de snapshot determinista y sensible al cambio, formato EIP-681 exacto, rechazo estricto polygon/USDT/solana, estructura Safe batch); 160/160 motor + 17/17 backend. **(h) E2E** contra contenedor dev: Empresa 46 "E2E OnChain" + periodo 32 liquidado + 3 contratistas (2 con wallet, 1 sin), TRM real 3206.86 (2026-07-23) hash `4a440c6ae3c0e8ad…`, totalCop 7.700.000 → 2401.10 USDC (Ana 4.200.000 COP → 1309.692347 USDC = 1309692347 unidades; Bob 3.500.000 → 1091.410289 USDC), "Sin Wallet" excluido correctamente, EIP-681 bien formado, Safe batch con 2 transactions, rechazo con `txHash` inválido ("No se encontró la transacción..."), rechazo por expiración forzada ("El lote expiró..."). Empresa 46 se conserva como fixture E2E permanente en dev (usuario lo decidió explícitamente); empresas reales id 6 y 7 intactas. `viem ^2.55.5` agregado a `apps/api`. **Fuera de scope permanente**: custodia de fondos, hot wallets, firma server-side, pago on-chain de salarios de empleados (CST art. 134-136).
- [x] **Fase 7 — Puesta en marcha: posicionamiento + landing viva + fichas Meta Ads**: primera ronda del entregable del prompt maestro (§16). Tres piezas encadenadas por el mismo hilo de marca: **(a) Bloque de posicionamiento** en `sdd/marketing/posicionamiento.md` — enunciado clásico, promesa de marca (`"La ley colombiana, convertida en un cálculo que puedes revisar tú mismo"`), territorio de tono con 5 pares "es/no es", los tres pilares canónicos (gratis+sin registro / cada cifra cita la ley / verificar-vs-liquidar), 12 objeciones con respuesta-mensaje repartidas entre las 4 audiencias, reglas duras de copy (no decir "IA", cifras en pesos, nombres inventados). **(b) Landing canónica** en `apps/web/src/lanzamiento/` — ruta `/lanzamiento` cableada en `main.tsx` antes de los otros portales. Ocho secciones del §16 §4: hero, problema, cómo funciona, prueba de confianza, caso real, puente B2B (con ancla `#empresas` y `#contadores` para retargeting), FAQ acordeón, cierre + footer legal. **Signature element:** `ReciboDemo.tsx` — comprobante de "Juan P., mesero" contra el motor con 5 líneas de cálculo real citando CST art. 179, 168 y Ley 15/1959; cada línea es click-expandible con la fórmula matemática (`4h × $8.104/h × 75%`) y el veredicto legal. Total `FALTA PAGARLE $48.545` grande en coral. Sistema de tokens específico para la landing sin romper el theme del producto (`index.css`): paleta del semáforo del motor (`--color-verde #0d9488`, `--color-ambar #f59e0b`, `--color-coral-fuerte #e11d48`) sobre papel (`#faf7f0`), tres tipografías (`Space Grotesk` display, `Inter` body ya existente, `JetBrains Mono` para cifras contables — el mono en los números es signature secundario), Google Fonts vía `@import` en el CSS. Mobile-first, `min-h-screen` con secciones que alternan cream/oscuro/cream para ritmo visual. Cero animación gratuita — la única "interacción" es el acordeón del recibo y el FAQ. Copy centralizado en el propio componente con `PILARES`/`PASOS`/`FAQ` como const para que un cambio de tono se resuelva en un archivo. **(c) Fichas de campañas Meta Ads** en `sdd/marketing/campanas-meta-ads.md` — las 4 campañas del §16 §5 listas para copiar al MCP de Meta Ads en otra sesión (no las crea este agente porque requieren cuenta publicitaria real y "espera confirmación explícita antes de activar", literal del §16 §5). Cada campaña con buyer persona, tabla de segmentación (ubicación, edad, intereses, comportamientos, audiencias personalizadas, lookalikes, exclusiones), placements Meta específicos, formato de creativos, tres variantes de gancho (dolor/curiosidad/cifra), copy del anuncio, CTA con URL con `utm_campaign` diferenciado, KPIs de éxito por 14 días. Distribución de presupuesto 60/15/20/5 documentada. **Pixel/eventos** — `apps/web/src/lanzamiento/tracking.ts` con contrato tipado `EventoMeta` (`verificacion_iniciada`, `verificacion_completada`, `discrepancia_detectada`, `registro_empresa`, `interes_partners`) y `trackEvento()` que hace no-op + `console.debug` en dev (se cablea a `fbq()` cuando el pixel esté conectado). Los CTAs de la landing ya llaman a `trackEvento` con `utm_campaign`/`origen` para atribución. Verificado: `tsc` limpio en web, la landing carga en `http://localhost:5173/lanzamiento` con hero + recibo demo interactivo (el `get_page_text` del browser preview confirma las 8 secciones renderizadas con el copy completo); 160/160 motor + 7/7 backend siguen verdes. Los screenshots del preview headless mostraron artefactos de captura en scrolls no-zero pero el DOM confirmó (via `elementFromPoint` + `getComputedStyle`) que las secciones oscuras/claras se pintan con los backgrounds correctos.
- [x] **Fase 6.2 — Hardening post-Fase 6.1: tests, tipoContrato, benchmark**: (a) **Vitest en `apps/api`** — configurado a solo tests puros (motores sin BD/HTTP); 7 tests para `calcularReciboLote` + `calcularRecibosContratistas` que cubren happy path, gate QA por-empleado (rechazo por salario ínfimo, rechazo por HE día/semana como severidad `error`), aprendiz lectivo/práctico exentos, lote vacío y separación de contratistas. (b) **Fix del gate QA para contratos exentos** — el pipeline rechazaba al aprendiz SENA por `IBC_FUERA_DE_RANGO` aunque la Ley 789/2002 art. 30 lo exime de cotizar. `DatosQA` gana `exentoDeCotizacion?: boolean`; `evaluarQA` salta `validarRangoIbc` + `validarNetoMinimo` cuando el flag es `true`; `liquidacionCalculo.ts` lo activa para todo `tipoContrato` que empieza con `aprendizaje_sena`. El resto de gates (deducciones, HE, decimales PILA, choque novedades) siguen corriendo. 7/7 backend + 160/160 motor. (c) **Benchmark del worker** — empresa temporal (id 45) con 600 y 1200 empleados fijos, medición contra el contenedor de dev; `TAMANO_LOTE` ahora es configurable por env `LOTE_LIQUIDACION` (default 50). Resultados (mediana de 3 corridas): con 600 empleados, `LOTE=25 → 188ms`, `LOTE=50 → 196ms`, `LOTE=100 → 180ms`, `LOTE=200 → 176ms`; con 1200 empleados `LOTE=50 → 370ms`. Diferencias entre tamaños son ≤20ms, escala lineal. Decisión: **mantener default 50** — el overhead por lote es despreciable frente al cómputo del motor, y con lotes de 50 el `progreso` se actualiza cada ~8% (buena UX) sin acercarse al comportamiento "todo o nada" que traería `LOTE=200`. A 1000+ empleados en producción no se justifica réplicas horizontales todavía; se revisita si `progreso` deja de sentirse fluido. Datos de prueba limpiados; empresas reales intactas. Notas: la observación de la v2.3 sobre `tipoContrato` "sin implementar" estaba desactualizada — UI, zod, motor y schema ya lo soportaban; el único gap real era el del gate QA que se corrigió aquí.
- [x] **Fase 6.1 — Liquidación Asíncrona Enterprise**: integración de `pg-boss` sobre el mismo Postgres de Supabase (sin Redis), procesamiento por lotes de 50 empleados con `conAuditoria` por lote, extracción del `PanelLiquidacion` con polling cada 3s vía `usePeriodoEstado`, y manejo de estado terminal parcial (`liquidado_con_rechazos`) que no bloquea la nómina del resto por un solo recibo con errores de QA. Verificación E2E de idempotencia (revertir + reencolar no duplica recibos, guard por `(periodoId, empleadoId)`) y prevención de duplicidad completada con éxito contra el contenedor. Ver commits `54d5c27` (M1: máquina de estados), `ff3c843` (M2: worker + gate), `6089a8e` (M3: UI + hook). Detalle técnico en las tres entradas anteriores.
- [x] **Listados de empresa: paginación + filtros server-side + SPA con react-router**: la app empresa deja de ser state-local y pasa a ser SPA con rutas — cada pestaña tiene su URL (`/empresa/auditoria`, `/empresa/periodos`, etc.), back/forward del navegador funcionan, deep-link se puede compartir. `apps/api/src/lib/paginacion.ts` centraliza `paginacionDeQuery`/`booleanoOpt`/`stringOpt`; los cuatro listados afectados (`auditoriaService`, `empleadosService.listarEmpleados`, `contratistasService.listarContratistas`, `periodosService.listarPeriodos`) devuelven `{ items, total, page, limit }` y aceptan filtros específicos (auditoría: `q, tabla, accion, desde, hasta`; empleados: `q, sedeId, activo, tipoContrato`; contratistas: `q, activo`; periodos: `estado, desde, hasta`). El scoping por sedes del analista sigue como hard filter — un filtro `sedeId` por query nunca puede ampliar el alcance. UI: nuevo directorio `apps/web/src/components/filtros/` con 4 componentes reutilizables — `useFiltrosUrl<T>(defaults)` (hook que sincroniza filtros ↔ URL params, elimina params iguales al default, usa `replace` para no ensuciar historial), `CampoBusqueda` (input con debounce 250ms + botón X), `SelectFiltro<T>` (con opción "Todas" que borra el filtro), `Paginador` (prev/N/next con contador, se oculta con una sola página). Cablado en las 4 pestañas: `AuditoriaEmpresa`, `DashboardEmpresa` (empleados — pierde el sumatorio "nómina base mensual" que ya vive en Costos, gana totales `activos`/`retirados` reales del server), `ContratistasEmpresa`, `PeriodosEmpresa` (filtros por estado + rango de fechas). `pnpm add react-router-dom` en `apps/web`; `main.tsx` envuelve `EmpresaApp` en `<BrowserRouter basename="/empresa">`; `SegmentedControl` del panel navega con `useNavigate` en vez de `setState`. Verificado en navegador: `/empresa/auditoria` carga (react-router resuelve, muestra AuthEmpresa por falta de sesión — comportamiento esperado); build de producción limpio. Reintros posibles luego: filtros/paginación en Discrepancias, Recibos y Staff empresarial cuando la escala lo pida.
...
- **Componentes en Rust (solo si aparece un cuello real, no como fundación)**: hoy TS puro cubre todo el hot path — el motor de reglas es determinístico y corre en milisegundos por lote, la verificación on-chain con viem es I/O-bound (RPC Base), y multi-país es más filas en `ReglaLegal`, no más CPU. Escenarios donde sí valdría extraer un binario/microservicio Rust puntual, medido antes:
  - **Indexer on-chain propio 24/7** sobre miles de wallets/eventos (alloy o ethers-rs) — cuando dependamos de estado on-chain histórico y no solo del receipt puntual del batch.
  - **ZK-proofs de nómina** ("probé que pagué X sin revelar Y", circom/halo2) — para reportes verificables sin exponer PII.
  - **Batches de decenas de miles de empleados/segundo** con reglas complejas — perfilar el TS primero; casi siempre el cuello está en la BD, no en el CPU JS.
  Ninguno justifica hoy el costo de dos toolchains, FFI, y partir el catálogo legal en dos lenguajes.
...
## 17 — Fase 8: Pago on-chain no-custodial (USDC en Base)

Cierra el ciclo de vida de un `PeriodoNomina` — antes de v2.5 el estado `pagado` era un enum nunca alcanzado. Ahora `liquidado`/`liquidado_con_rechazos` → `pagado` requiere una transacción real en Base verificada por el servidor. Alcance permanente **fase 1**: solo contratistas de servicios; empleados quedan siempre en flujo COP (CST 134-136).

### Principios rectores (no negociables)

1. **NomiCheck jamás custodia fondos.** El servidor no tiene, ni tendrá, llaves privadas de empresa. Genera artefactos de firma (EIP-681, Safe JSON) y verifica receipts on-chain. El empleador firma desde su propia wallet (MetaMask, Rabby, Safe multifirma). Este límite existe para (a) evitar licencia de la SFC y (b) eliminar la superficie de fondos ajenos comprometibles en el servidor.
2. **USDC nativo Circle, no bridged.** `resolverRedPago` acepta *solamente* `{red: "base", token: "USDC", tokenAddress: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"}`. Cualquier otro par lanza `ErrorRedNoSoportada` con mensaje explícito. Multi-red (Polygon, Arbitrum, Solana) queda como configuración futura en `ReglaLegal`, no como refactor.
3. **Salario de empleados: siempre COP.** CST art. 134-136 exige moneda legal. `Empleado.walletAddress` existe pero es solo referencial (constancia); `generarBatchPago` está codificado para iterar `Contratista` únicamente. Un empleado con `walletAddress` **jamás** aparece en un `PagoItem`.
4. **Tasa trazable con hash.** `capturarTasaSnapshot` congela TRM oficial + prima parametrizable en un snapshot con `hash = sha256(JSON canónico)`. Un tercero (auditor, contratista, tribunal) puede reconstruir el hash con los mismos insumos y verificar que la tasa citada en el recibo es la real.
5. **Ventana de expiración.** El batch expira si no se firma en `pago_onchain_ventana_horas` (default 6h). Vencido → regenerar con tasa fresca. Protege al contratista de un lote firmado semanas después con una tasa vieja perjudicial.
...
```
  Periodo (liquidado)
        │
        ▼
  generarBatchPago  ──► capturarTasaSnapshot (TRM + hash)
        │                           │
        │                           ▼
        │             copAUnidadesUsdc (6 decimales, half-away)
        │                           │
        ▼                           ▼
  BatchPago + PagoItem[]  ──► linkEip681 (por item)
        │                    safeBatchJson (batch multifirma)
        ▼
  [ Empleador firma desde su wallet ]
        │
        ▼
  verificarBatchPago(txHash)
        │
        ├─► viem: getTransactionReceipt + parseEventLogs (Transfer)
        ├─► match exacto (destinoWallet, unidades) por item
        ├─► logs consumidos una sola vez
        │
        ▼
  Periodo.estado = "pagado" (concurrencia optimista con version)
```

### Parametrización país (expansión)

El catálogo actual de `ReglaLegal` es Colombia implícito. Para expandir:

- **Enfoque diferido, no reactivo**: no se agrega columna `pais` a `ReglaLegal` en v2.5 — la decisión de si el ámbito se modela como namespace de claves (`co.pago_onchain_red`) o como columna nueva (`ambito`) se toma cuando entre el segundo país, con el requisito real en mano. Documentado como decisión pendiente para no diseñar sobre hipótesis.
- **Claves ya parametrizables (v2.5)**: `pais_moneda`, `pago_onchain_red`, `pago_onchain_token`, `pago_onchain_token_address`, `pago_onchain_prima_pct`, `pago_onchain_ventana_horas`.
- **Contratos legales que NO son parametrizables** (viven en `pagosService`, no en `ReglaLegal`): la exclusión de empleados del batch (CST art. 134-136 aplica a Colombia; otros países pueden tener otras leyes), el patrón EIP-681, la estructura del Safe JSON. Se moverán a `ReglaLegal` cuando exista el segundo país que lo justifique.

### Fuera de scope permanente

- Custodia server-side, hot wallets, firma automática.
- Swap COP↔USDC dentro de la app; off-ramp; integraciones con exchanges.
- Pago on-chain de salarios de empleados en Colombia.
- Multi-red (fase 2 vía `ReglaLegal`, no refactor).
- Integración automática con orders de Execution Market (webhook → pg-boss) — fase 2 del doc `execution_market/docs/04_alcance_nomicheck_launch.md`.
