...
- [x] **Tipos de contrato completos en el modo empresa + calculadoras anónimas por concepto**:
  - **tipoContrato lado empresa**: el motor y el verificador anónimo ya soportaban los 6 valores, pero el formulario de empleado, el tipo de `apiEmpresa.ts` y el enum zod de `validation/empresa.ts` solo aceptaban 3 — se amplían a los 6 (`fijo`, `obra_labor`, `tiempo_parcial` nuevos en empresa). Sin migración (columna String). `costosService`/`liquidacionService` ya tratan cualquier no-aprendiz como ordinario (recibo completo + provisión de prestaciones), que es lo correcto. Nota: `advertenciaTerminoNoIndefinido` viaja en `ResultadoNomina.advertencias` de las calculadoras pero el `ReciboPago` persistido guarda solo las líneas — la advertencia no queda en el recibo de empresa (sí en el verificador anónimo).
