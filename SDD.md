...
- **Contratistas de servicios en el modo empresa**: `CalculadoraServicios` existe en el motor y en `/nomina/calcular`, pero no hay una entidad de captura en el modo empresa (no es `Empleado` — no es contrato laboral). Requeriría un modelo propio (`Contratista`?) en vez de forzarlo dentro de `Empleado`.
