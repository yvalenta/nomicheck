...
**Estado al 2026-09-10:** los pasos 2–6 de abajo están construidos en la rama `tarea/dx402-punto-2` (commit `6658792`). Lo que falta, en orden:

1. **Revisión de la sesión madre** (ninguna sesión Fable leyó el código entero todavía; lo escribieron sonnet y lo refutó opus): `lib/x402MuroDurable.ts`, `lib/llaveDelPagador.ts`, `lib/anclajeDiferido.ts`, `lib/sobre.ts`, y el diff de `x402Muro.ts`/`x402Config.ts`. Decidir sobre los cuatro puntos «fuera del brief» de la bitácora (deploy.sh, `retencionExterna`, pricing/openapi, GET v1).
2. **Tercera ronda de refutación** sobre las 13 reparaciones de la ronda 2 (lentes dinero / identidad / protocolo, `agentType: refutador`), y cazar el flaky (correr la suite ~20 veces con `npx vitest run` y guardar la salida roja).
3. **Merge a `main`** desde el worktree y desmontarlo (`git worktree remove` + `git branch -d`), regla del worktree de la Línea Roja.
4. **Yonatan (lista 2):** crear la llave Ed25519 del sobre (`openssl genpkey -algorithm ed25519`), ponerla en el `.env` de producción como `NOMICHECK_SOBRE_SIGNING_KEY_PEM`, y `X402_RED` con `avalanche`; GO de despliegue. Sin esos dos valores `deploy.sh` aborta a propósito y, si pasara, la API entera no arranca.
5. **Compra real** (Yonatan): `testigo/tareas/2026-09-08-compra-real.md`, avisando allá que la red es **Avalanche** (hot wallet con USDC de C-Chain `0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E`, no Base/Monad), `--sobre-key https://nomicheck.ynt.codes/api/batch/verificar/durable/sobre-publickey`, `--expected-signer 0x7bC4b9cc90a057A95A0F5a8F93C3e3996EE4e0DF`.
