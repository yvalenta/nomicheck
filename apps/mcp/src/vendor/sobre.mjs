...
//
// ┌─────────────────────────────────────────────────────────────────────────┐
// │ COPIA VENDORIZADA — NO EDITAR ACÁ.                                      │
// │ El original vive en el repo público `sobre` (CC0, cero dependencias,    │
// │ Node 18+). El servidor MCP lo consume desde acá para que                │
// │ `nomicheck_verificar_sobre` verifique OFFLINE sin depender de un repo   │
// │ hermano ni de un paquete publicado. Los cambios van allá y se copian:   │
// │ dos versiones de la regla que decide QUÉ BYTES SE FIRMAN es la clase    │
// │ de deriva que nadie nota hasta que un comprador audita.                 │
// └─────────────────────────────────────────────────────────────────────────┘
